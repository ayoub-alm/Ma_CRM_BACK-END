...
public class ContractDocService {
    ...
    public byte[] generateContractDocx(StorageContract contract) throws IOException {
        try (
             InputStream inputStream = new ClassPathResource("templates/Contrat.docx").getInputStream();
             XWPFDocument document = new XWPFDocument(inputStream);
             ByteArrayOutputStream outputStream = new ByteArrayOutputStream()) {

            Map<String, String> placeholders = preparePlaceholders(contract);
            ...
            if (tables.size() >= 3) {
                fillTable(tables.get(0), extractDepotage(contract));
                fillTable(tables.get(1), extractLogisticsOperations(contract));
                fillTable(tables.get(2), extractValueAddedServices(contract));
            }
    ...
    private Map<String, String> preparePlaceholders(StorageContract contract) {
        ...
        placeholders.put("Nombre_de_SKU", Optional.ofNullable(contract.getNumberOfSku()).map(String::valueOf).orElse("0"));
        ...
        placeholders.put("Delais_de_paiement", Optional.ofNullable(contract.getPaymentDeadline()).map(String::valueOf).orElse("N/A"));
